...
**Historical reference.** This bundle documents how Tony was run on Matt's laptop *before* the migration to the server. It is not what's running today — but it's useful reference for understanding the architecture.

It's the full contents of `C:\openclaw\ClawLauncher-Windows` from Matt's laptop: the launcher GUI plus everything needed to install its Windows dependencies.
...
| `launcher.log` | Runtime log from Mar 7, 2026 — shows VC++ redist install, failed Node.js MSI (exit 1603), Chocolatey bootstrap, and `openclaw doctor` invocations. A good trace of what the launcher actually does on startup. |
...
- `ClawLauncher.exe` is a Go-based GUI wrapper, not the runtime itself. The actual runtime is the `openclaw` npm package (Node.js), which is cross-platform (the OpenClaw README explicitly supports macOS, Linux, and Windows via WSL2). Useful context for understanding how Tony can run on different hosts.
